...
def install_and_import(package):
    try:
        ...
        if shutil.which("pip3"):
            ...
            print(f"{GREEN}[i] pip3 not found, installing python3-pip...{RESET}")
...
def interactive_menu():
    ...
    choices = input(GREEN + "Enter choices (comma separated, e.g. 1,3,5): " + RESET).strip()
    selected = set()
...
def scan_xss(url, param, method):
    print(f"{GREEN}[i] Starting XSS test...{RESET}")
    payload = "<script>alert('XSS')</script>"
    params = {param: payload}
    r = get_request(url, params, method)
    if r and payload in r.text:
        print(f"{GREEN}[!] Possible XSS vulnerability detected on parameter '{param}'.{RESET}")
    else:
        print(f"{GREEN}[-] No XSS vulnerability detected on parameter '{param}'.{RESET}")

def scan_sqli(url, param, method):
    print(f"{GREEN}[i] Starting SQL Injection test...{RESET}")
    payload = "' OR '1'='1"
    params = {param: payload}
    r = get_request(url, params, method)
...
def scan_open_redirect(url, param, method):
    print(f"{GREEN}[i] Starting Open Redirect test...{RESET}")
    ...
    try:
        ...
        if r and "Location" in r.headers and redirect_test in r.headers["Location"]:
            print(f"{GREEN}[!] Possible Open Redirect vulnerability detected on parameter '{param}'.{RESET}")
        else:
...
def scan_security_headers(url):
    print(f"{GREEN}[i] Checking Security Headers...{RESET}")
    try:
        r = requests.get(url, timeout=10)
        headers = r.headers
        needed = {
        ...
        for h, val in needed.items():
            if h not in headers:
                ...
                missing.append(h)
        if missing:
            print(f"{GREEN}[!] Missing or weak security headers: {', '.join(missing)}{RESET}")
        else:
...
def scan_clickjacking(url):
    print(f"{GREEN}[i] Checking Clickjacking protection...{RESET}")
    try:
        r = requests.get(url, timeout=10)
        if "X-Frame-Options" in r.headers or "Content-Security-Policy" in r.headers:
            print(f"{GREEN}[-] Clickjacking protection headers found.{RESET}")
        else:
...
def scan_directory_listing(url):
    print(f"{GREEN}[i] Checking Directory Listing...{RESET}")
    if not url.endswith("/"):
        url += "/"
    try:
        r = requests.get(url, timeout=10)
        if "Index of /" in r.text or "<title>Directory listing for" in r.text.lower():
            print(f"{GREEN}[!] Possible Directory Listing enabled at: {url}{RESET}")
        else:
...
def main():
    ...
    url = input(f"{GREEN}Enter target URL (e.g. http://example.com/page): {RESET}").strip()
    if not url.startswith("http://") and not url.startswith("https://"):
        print("[!] Please enter a valid URL starting with http:// or https://")
        sys.exit(1)
    method = input(f"{GREEN}Request method (GET/POST): {RESET}").strip().upper()
    if method not in ["GET", "POST"]:
        print("[!] Invalid method, defaulting to GET.")
        method = "GET"
    param = input(f"{GREEN}Enter parameter name to test (e.g. id, q, search): {RESET}").strip()
